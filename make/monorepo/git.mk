...
git-diff-subrepo: bootstrap-infra subrepo-check
...
git-fetch-subrepo: bootstrap-infra subrepo-check
	$(call exec,git fetch --prune $(REMOTE))

.PHONY: git-stash
git-stash: bootstrap-infra git-status
	if [ ! $(STATUS) -eq 0 ]; then \
		$(call exec,git stash); \
	fi

.PHONY: git-status
git-status: bootstrap-infra
	$(eval DRYRUN_IGNORE := true)
	$(eval STATUS := $(shell $(call exec,git status -uno --porcelain 2>/dev/null |wc -l)))
	$(eval DRYRUN_IGNORE := false)

.PHONY: git-unstash
git-unstash: bootstrap-infra
	$(eval STATUS ?= 0)
	if [ ! $(STATUS) -eq 0 ]; then \
...
branch-create-upstream-%: bootstrap-infra update-upstream
...
.PHONY: branch-delete
branch-delete: bootstrap-infra update-upstream
...
branch-merge-upstream-%: bootstrap-infra update-upstream
...
tag-create-upstream-%: bootstrap-infra update-upstream
ifneq ($(words $(TAG)),0)
	$(call exec,git checkout $*)
...
tag-merge-upstream-%: bootstrap-infra update-upstream
ifneq ($(words $(TAG)),0)
