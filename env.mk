...
define .env ## update $(env_file) file with vars from $(env_dist) except those already set in system env
	...
	if [ -f "$(env_dist)" ]; then \
		touch $(env_file); \
		printenv |awk -F '=' 'NR == FNR { A[$$1]; next } !($$1 in A)' - $(env_dist) |awk -F '=' 'ARGV[1] == FILENAME { A[$$1]; next } !($$1 in A)' $(env_file) - |(IFS=$$'\n'; env $(env_reset) $$(env |awk -F '=' 'NR == FNR { A[$$1]; next } ($$1 in A)' $(env_dist) - |cat - $(env_dist) |sed -e /^$$/d -e /^#/d |awk -F "=" '!seen[$$1]++') awk '{while(match($$0,"[$$]{[^}]*}")) {var=substr($$0,RSTART+2,RLENGTH -3);gsub("[$$]{"var"}",ENVIRON[var])} print}') >> $(env_file); \
	fi
endef
